...
export function ContractDisplay(props: { contract: Contract }) {
    ...
    return <Box flexDirection="column">
        ...
        <Row headWidth={6} head="colo" value={props.contract.colocate_with.length > 0 ? props.contract.colocate_with.join(", ") : "-"} />
...
export function ContractList(props: { contracts: Contract[] }) {
    ...
    return <Box flexDirection="column" gap={1} paddingBottom={1}>
        {props.contracts.map((contract) => <ContractDisplay contract={contract} key={contract.id} />)}
    </Box>;
}
