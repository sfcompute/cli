...
type Order = Awaited<ReturnType<typeof getOrder>> | Awaited<ReturnType<typeof placeSellOrder>>;
...
async function sellOrderAction(options: SfSellOptions) {
    ...
    if (isNaN(size) || size <= 0) {
        return logAndQuit(`Invalid number of accelerators: ${options.accelerators}`);
    }
    ...
    const endDate = roundEndDate(dayjs(startDate).add(durationSeconds, "seconds").toDate()).toDate();
        ...
        <Box gap={1} flexDirection="column">
            ...
            {order && order.status === "open" && (
                <Box paddingY={1} paddingX={2} flexDirection="column" gap={1}>
                    <Text>Your order is open, but not filled. You can check its status with... </Text>
                    <Box paddingLeft={2}>
...
export async function placeSellOrder(options: {
    ...
    const realDurationHours =
        dayjs(options.endsAt).diff(dayjs(options.startAt === "NOW" ? new Date() : options.startAt)) /
        3600 /
        1000;
    const totalPrice = getTotalPrice(options.price, options.quantity, realDurationHours);
    invariant(totalPrice == Math.ceil(totalPrice), "totalPrice must be a whole number");
    ...
    const { data, error, response } = await api.POST("/v0/orders", {
        body: {
            ...
            start_at: options.startAt === "NOW" ? "NOW" : options.startAt.toISOString(),
